...
//*************************************************Bernice's tests for sales by year
// Test the sales report API
describe('Apre Sales Report API - Years', () => {
  beforeEach(() => {
    mongo.mockClear();
  });

  // Test the sales/regions endpoint
  it('should fetch a list of distinct sales years', async () => {
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
        distinct: jest.fn().mockResolvedValue(['2021', '2022', '2023', '2024'])
      };
      await callback(db);
    });

    const response = await request(app).get('/api/reports/sales/years'); // Send a GET request to the sales/regions endpoint

    expect(response.status).toBe(200); // Expect a 200 status code
    expect(response.body).toEqual(['2021', '2022', '2023', '2024']); // Expect the response body to match the expected data
  });


  // Test the sales/years endpoint with no years found
  it('should return 200 with an empty array if no years are found', async () => {
    // Mock the MongoDB implementation
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
    ...
    // Make a request to the endpoint
    const response = await request(app).get('/api/reports/sales/years');
...
describe('Apre Sales Report API - Sales by Year', () => {
  ...
  // Test the sales/years/:year endpoint
  it('should fetch sales data for a specific year, grouped by salesperson', async () => {
    ...
    const response = await request(app).get('/api/reports/sales/years/2023'); // Send a GET request to the sales/regions/:region endpoint
    expect(response.status).toBe(200); // Expect a 200 status code
  ...
  it('should return 200 and an empty array if no sales data is found for the year', async () => {
    // Mock the MongoDB implementation
    mongo.mockImplementation(async (callback) => {
      const db = {
        collection: jest.fn().mockReturnThis(),
    ...
    // Make a request to the endpoint
    const response = await request(app).get('/api/reports/sales/years/unknown-year');
